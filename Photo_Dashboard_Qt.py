"""
Photo_Dashboard_Qt.py
Application autonome de retouche photo astronomique.

Dépendances :
    pip install PyQt5 matplotlib numpy astropy photutils scipy pillow imageio astroalign

Usage :
    python Photo_Dashboard_Qt.py
"""
...
# --- Import du module de traitement (inchangé) ---
from Photo_Lab import PhotoLab


# ─────────────────────────────────────────────────────────────────────────────
# WORKER : exécution hors thread UI
# ─────────────────────────────────────────────────────────────────────────────

class Worker(QThread):
    """Thread générique pour les opérations longues."""
    finished = pyqtSignal(object)   # résultat (peut être None)
...
# ─────────────────────────────────────────────────────────────────────────────
# CANVAS MATPLOTLIB (plot principal + histogramme)
# ─────────────────────────────────────────────────────────────────────────────

class AstroToolbar(NavigationToolbar2QT):
    """Toolbar matplotlib épurée : sans Save ni Setup, icônes claires."""
    # Seuls les boutons à conserver (None = séparateur)
...
# ─────────────────────────────────────────────────────────────────────────────
# WIDGET SLIDER AVEC LABEL (remplace ipywidgets.FloatSlider)
# ─────────────────────────────────────────────────────────────────────────────

class LabeledSlider(QWidget):
    """Slider flottant avec label, valeur affichée et callback."""
    ...
    def __init__(self, label, vmin, vmax, vstep, vdefault, handle_color=None):
        ...
        if handle_color:
            self.slider.setStyleSheet(f"""
                ...
                QSlider::sub-page:horizontal {{
                    background: {handle_color}; height: 4px; border-radius: 2px;
                }}
            """)
        layout.addWidget(self.slider)

        # Valeur numérique (50px)
        ...
        # Label mis à jour en temps réel, signal émis seulement au relâchement
    ...
    def _on_drag(self, i):
        """Met à jour uniquement le label pendant le drag."""
        self.val_lbl.setText(f"{self._int_to_val(i):.3g}")

    def _on_release(self):
        """Émet le signal (déclenche le refresh) au relâchement."""
...
class PhotoDashboardQt(QMainWindow):
    def __init__(self):
        ...
        # Thread actif (pour éviter la GC)
        self._worker = None
        self._pending_refresh = None   # (update_masks, log_msg) en attente

        # Debounce : on attend 300ms d'inactivité avant de lancer le refresh
    ...
    def _apply_dark_theme(self):
        self.setStyleSheet("""
            ...
            QSlider::handle:horizontal {
                background: #888; border: 1px solid #aaa;
                width: 14px; height: 14px;
    ...
    def _build_ui(self):
        ...
        self.chk_grad = QCheckBox("Gradient removal")
        self.chk_grad.toggled.connect(lambda v: self._refresh(False, f"Gradient removal: {'ON' if v else 'OFF'}"))
        self.chk_masks = QCheckBox("Show masks")
        self.chk_masks.toggled.connect(lambda v: self._refresh(False, f"Show masks: {'ON' if v else 'OFF'}"))
        ...
        # Sliders Min / Max sous l'histogramme
        self.sl_level_min = LabeledSlider('Min', 0.0, 0.5,  0.001, 0.0, handle_color='#ffffff')
        self.sl_level_max = LabeledSlider('Max', 0.5, 1.0,  0.001, 1.0, handle_color='#ffffff')
        ...
        slider_defs = [
            # (groupe, nom, vmin, vmax, vstep, vdefault, handle_color)
            ("COLOR BALANCE",
            ...
            ("COSMETICS",
             [('Stretch',    0.01, 0.1,  0.001, 0.01, None),
        ...
        for grp_name, items in slider_defs:
            ...
            for name, vmin, vmax, vstep, vdef, hcolor in items:
                ...
                s.valueChanged.connect(lambda v, n=name, m=is_mask: self._refresh(m, f"Setting: {n}"))
    ...
    def _log(self, msg):
        # Peut être appelé depuis un thread secondaire → signal-safe via Qt
    ...
    def _on_load_click(self):
        ...
        # Afficher les noms de fichiers dans le label
        names = [os.path.basename(p) for p in paths]
        summary = f"{len(paths)} file(s) : {', '.join(names)}"
        self.files_label.setText(summary)

        self._set_status(True)
        self._log("Loading files…")
        self.btn_load.setEnabled(False)

        self._worker = Worker(self.lab.load_files, paths)
        self._worker.finished.connect(self._on_load_done)
        self._worker.error.connect(lambda e: (self._log(f"Error: {e}"), self._set_status(False)))
        self._worker.start()
    ...
    def _refresh(self, update_masks=False, log_msg=None):
        """Entrée publique : accumule les demandes et lance après 300ms d'inactivité."""
        if self.lab.calibrated is None:
            return
        # On mémorise la demande la plus récente (update_masks = True l'emporte)
        prev = self._pending_refresh
        self._pending_refresh = (
            update_masks or (prev[0] if prev else False),
            log_msg or (prev[1] if prev else None)
        )
        self._debounce_timer.start(300)   # repart à 0 à chaque appel

    def _do_pending_refresh(self):
        """Appelé par le timer : lance le worker si aucun n'est actif."""
        if self._pending_refresh is None:
            return

        # Si un worker tourne encore, on repose le timer et on attend
        ...
        # Capture des valeurs des sliders dans le thread UI avant de passer au worker
        params = dict(
            update_masks = update_masks,
            show_masks   = self.chk_masks.isChecked(),
            masks        = self.lab.masks,
        ...
        def _do_refresh():
            ...
            if params['show_masks'] and self.lab.masks:
                ...
                # Application du clipping Min/Max + renormalisation
        ...
        self._worker = Worker(_do_refresh)
    ...
    def _update_histogram(self, stretch=None, bp_ratio=None, level_min=None, level_max=None):
        if stretch is None:   stretch   = self.sliders['Stretch'].value
        ...
        if len(useful) > 0:
            ...
            # Lignes Min (blanche) et Max (jaune)
            # level_min/max sont en [0,1] ; l'histo est tracé sur [h_min, h_max]
            # On convertit : position = h_min + level * (h_max - h_min)
            span = h_max - h_min
    ...
    def _export(self):
        ...
        # Capture de tous les réglages dans le thread UI
        ...
        self._worker = Worker(_do_export)
        self._worker.finished.connect(lambda b: (
            self._log(f"Export done: {b}.tiff  +  {os.path.basename(b)}.meta"),
            self._set_status(False)
        ))
        self._worker.error.connect(lambda e: (self._log(f"Export error: {e}"), self._set_status(False)))
        self._worker.start()
